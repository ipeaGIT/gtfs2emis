...
#
emi2 <- emi 
emi2 <- data.table::as.data.table(emi2)[,`:=`(EM_CO = sum(EM_CO),EM_NOx = sum(EM_NOx)),by = .(id,tipo_veiculo)][,.SD[1],by = .(id,tipo_veiculo)] 
...
emi2$title <- paste0("Total emissions \n for ",stringr::str_to_sentence(emi2$tipo_veiculo)," bus")
i = 1
pf <- list()
for(v in 1:10){
  emi1 <- emi2[emi2$tipo_veiculo %in% unique(emi2$tipo_veiculo)[v],]
  pf[[i]] <- ggplot() +
...
pff <- (pf[[1]] + pf[[2]] + pf[[3]] + pf[[4]] + pf[[5]] + pf[[6]] + pf[[7]] + pf[[8]] + pf[[9]] + pf[[10]])
ggsave(plot = pff,filename = paste0("figures/CO_total_vei.jpg"),
       width = 18,height = 15,units = "cm",scale = 2,dpi = 300)
