...
ef_ub_ch4 <- vein::ef_hdv_scaled(dfcol = ub_ch4, 
                            ...
                            gr = 0, l = 0.5, p = "CH4")
# hex reading
hex_grid <- readRDS("data/hex/2304400_09.rds") %>% st_transform(31983) %>% st_sf()
...
mapview(dt$geometry)+mapview(hex_city,zcol="emi_nox")+
  mapview(hex_grid$geometry,alpha.regions=0)
