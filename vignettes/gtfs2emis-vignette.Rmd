...
```
  ...
  records, using `gtfs2gps::gtfs2gps()`. The example below uses a sample of the GTFS data from São Paulo municipality, in Brazil. In this example we will only process the trips of two `shape_id`s to speedup processing time.
...
Now that we have route segment-level information on vehicles, distances and emission factors, emissions levels can be estimated as a product between distances (units `km`) and emission factors (units `g/km`). Using the `emis` function we can can estimate emissions at high spatial and temporal resolutions.
...
breaks <- minute_emissions$departure_time[seq(1,nrow(hour_emissions),length.out = 5)]

ggplot(data = minute_emissions) +
  geom_line(aes(group = 1, x = departure_time, y = as.numeric(BR_CO_total)/1000), color='gray50') +
  geom_line(aes(group = 1, x = departure_time, y = as.numeric(EU_CO_total)/1000)) +
...
# 
# 
# spo_emis[, .(BR_CO2_total=sum(BR_CO2_total),
#              EU_CO_total=sum(EU_CO_total)), by= list(geometry)]
# 
...
# # aggregate
# 
# a <- st_join(spo_sf, grid_gps)
# 
# 
# a %>% 
#   mutate(hour_of_day = hour(as.POSIXct(strptime(departure_time, "%m/%d/%Y %H:%M:%S")))) %>% 
