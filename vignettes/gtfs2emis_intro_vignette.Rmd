...
```{r, message = FALSE}
emi_list$time_column <- emi_list$tp_model$timestamp  #[666] precisa disso? não dá pra colocar isso dentro da funcao emis_summary() ?
...
ggplot(data=emi_by_time) +
  ...
  labs(y="Total emissions (Kg)", x="Hour of the day") +
  facet_wrap(~pollutant, scales = "free", nrow = 2) + # [6666] checar scales free
  theme_minimal()
...
```{r,eval=FALSE, message = FALSE}
emi_list$time_column <- emi_list$gps$timestamp  #[666] precisa disso? não dá pra colocar isso dentro da funcao emis_to_dt() ?
...
```{r, message = FALSE}
# create spatial grid
mygrid <- sf::st_make_grid(emi_list$tp_model$geometry, cellsize = 0.25 / 102.47, square = FALSE)
mygrid <- sf::st_sf(data.frame(id=1:length(mygrid), geom=mygrid))

mapview::mapview(emis_grid$geometry) + mapview::mapview(emi_list$tp_model$geometry)
```



```{r, eval=FALSE, message = FALSE, fig.width=4,fig.height=4}

# we add info of emissions into gps file
emi_list$tp_model <- cbind(emi_list$tp_model,emi_list$emi) # isso é necessário? Não dá pra joga pra dentro da funcao? Alem disso, seria melhor fazer isso como uma operacao data.table := nao ?
