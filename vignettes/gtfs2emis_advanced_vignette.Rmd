...
This vignette introduces the main functions of the `gtfs2emis` package. We will be using the following packages:
...
```{r, eval = TRUE, message = FALSE, fig.width=3,fig.height=3}
...
```{r, eval = TRUE, message = FALSE}
fleet_data_ef_cetesb <- data.frame("veh_type" = c("BUS_MICRO_D"
                                                              ,"BUS_URBAN_D"
                                                              ,"BUS_ARTIC_D")
...
```{r, eval = TRUE, message = FALSE}
grid_gps <- sf::st_make_grid(sf_emis$gps, cellsize = 0.25 / 102.47, square = FALSE)
grid_gps <- sf::st_as_sf(grid_gps)
```

```{r, eval = TRUE, message = FALSE, fig.width=4,fig.height=4}
names(sf_emis$emi)
...
```{r}
fleet_data_ef_europe <- data.frame("veh_type" = rep(c("Ubus Midi <=15 t"
                                                                  ,"Ubus Std 15 - 18 t"
                                                                  ,"Ubus Artic >18 t"),2)
