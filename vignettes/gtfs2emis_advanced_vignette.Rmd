...
`gtfs2emis` is an R package to estimate hot exhaust emissions of public transport systems using data in the General Transit Feed Specification (GTFS) format. As mentioned in the [introductory vignette](add_link), `gtfs2emis` has two core functions: `transport_model()` and `emission_model()`. However, there are several advanced features that can be used to conduct a more sophisticated and rigorous emission model of public transport systems. This vignette demonstrates some of these advanced features in a step-by-step reproducible example.


In this vignette, we will be using a small sample data for the city of Curitiba, Brazil. In this example, we'll be estimating `CO2` and `PM10` emissions of bus services on business days.
