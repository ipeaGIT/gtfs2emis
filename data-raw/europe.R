...
# 2) EMEP/EEA for 2013 and 2009 were also used to add Fuel Consumption 
...
link2013 <- "https://www.eea.europa.eu/publications/emep-eea-guidebook-2013/part-b-sectoral-guidance-chapters/1-energy/1-a-combustion/1-a-3-b-road-transport-annex-hdv-files.zip/at_download/file"
a <- tempdir()
zipfile2013 <- paste0(a,"/1.A.3.b.i-iv Exhaust emissions from road transport annex HDV files.zip")
download.file(url = link2013,destfile = zipfile2013)
unzip(zipfile = zipfile2013, exdir = a)
filepath2013 <- paste0(a,"/1.A.3.b.i-iv Exhaust emissions from road transport data annex Sept2014 update.xlsx")
openxlsx::getSheetNames(filepath2013)
#eq2013 <- openxlsx::read.xlsx(xlsxFile = filepath2013,sheet = "HDV - BUS Equations") %>% 
#  data.table::setDT()
emep2013 <- openxlsx::read.xlsx(xlsxFile = filepath2013,sheet = "HDVs - BUS") %>% 
  data.table::setDT()
...
emep2013 <- emep2013[Sector %in% "Buses" & 
                       `Sub-Sector` %like% "Bus" &
                       Pollutant %in% "FC",]
emep2013[Technology %like% "SCR", Technology := "SCR"]
emep2013[Technology %like% "EGR", Technology := "EGR"]
emep2013[Technology %nlike% "EGR" & Technology %nlike% "SCR", Technology := NA]
emep2013[,`:=`(`50` = NULL,
               R2 = NULL,
               EF = NULL,
               RF = 0,
               ...
               Mode = NA,
               k = 1)]
emep2013[Euro.No. == 5, Euro.Standard := "Euro V"]
emep2013[Euro.No. == 6, Euro.Standard := "Euro VI"]
emep2013[,Euro.No. := NULL]
emep2013[,Load := Load / 100]
emep2013[is.na(Load), Load := 0.5]
data.table::setnames(emep2013,"Slope","Road.Slope")
emep2013[is.na(Road.Slope), Road.Slope := 0.0]
# fix formula names
# manual edit using formula below
# emep2013[,.SD[1],by = .(Formula)][,.(Equation,Formula)] %>% dput()
...
emep2013[formulas_dt,on = "Equation", Formula := i.Formula]
# rename files, so it matches with 2019
head(emep2013,1)
dim(emep2013)
head(emep2019,1)
dim(emep2019)
data.table::setnames(emep2013,old = c("Sector","Sub-Sector",
...
#
fileco2 <- data.table::copy(emep2013)[,Pollutant := "CO2"]
fileco2[,k := 44.011 / (12.001 * 1.86 + 16*0 ) ]
# rbind2013

emep2013 <- data.table::rbindlist(list(emep2013,fileco2))
...
# since it has already been used in newer EF database (2013)
...
#
europe <- data.table::rbindlist(list(emep2009,emep2013,emep2019))
europe[Euro.Standard %in% "Euro VI", Technology := "DPF+SCR"]
#
# rename Function_ID
