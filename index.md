...
1.  [`transport_model()`](https://ipeagit.github.io/gtfs2emis/reference/transport_model.md)
    converts GTFS data into a GPS-like table with the space-time
    positions and speeds of public transport vehicles. The only input
    required is a `GTFS.zip` feed.

2.  [`emission_model()`](https://ipeagit.github.io/gtfs2emis/reference/emission_model.md)
...
3.  [`emis_to_dt()`](https://ipeagit.github.io/gtfs2emis/reference/emis_to_dt.md)
    to convert the output of
    [`emission_model()`](https://ipeagit.github.io/gtfs2emis/reference/emission_model.md)
    from `list` to `data.table`.
4.  [`emis_summary()`](https://ipeagit.github.io/gtfs2emis/reference/emis_summary.md)
    to aggregate emission estimates by the time of the day, vehicle
    type, or road segment.
5.  [`emis_grid()`](https://ipeagit.github.io/gtfs2emis/reference/emis_grid.md)
...
[`transport_model()`](https://ipeagit.github.io/gtfs2emis/reference/transport_model.md)
function to convert GTFS data into a GPS-like table, so that we can get
the space-time position and speed of each vehicle of the public
transport system at high spatial and temporal resolutions.
...
[`emission_model()`](https://ipeagit.github.io/gtfs2emis/reference/emission_model.md)
function to estimate hot exhaust emissions of our public transport
system. Here, the user needs to pass the results from
[`transport_model()`](https://ipeagit.github.io/gtfs2emis/reference/transport_model.md),
some fleet data as described above, and select which emission factor
model and pollutants should be considered (see the options available
below). The output from
[`emission_model()`](https://ipeagit.github.io/gtfs2emis/reference/emission_model.md)
is a `list` with several `vectors` and `data.frames` with emission
...
- Brazil
  - CETESB: 2019 model from the Environmental Company of Sao Paulo
    (CETESB)
- Europe
